...
export function VoiceRecorder({ initialDate }: { initialDate: string }) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      workerRef.current = new Worker(
        new URL("../lib/sentiment-worker.ts", import.meta.url),
        {
          type: "module",
        },
      );
      reflectionWorkerRef.current = new Worker(
        new URL("../lib/reflection-worker.ts", import.meta.url),
        {
          type: "module",
        },
      );
        ...
        if (data.status === "success") {
          ...
          if (data.data.status === "downloading") {
            setLlmProgress(
              t("downloading", {
                file: data.data.file,
                progress: Math.round(data.data.progress || 0),
              }),
            );
          } else if (data.data.status === "done") {
            setLlmProgress(t("downloadComplete", { file: data.data.file }));
          }
        } else if (data.status === "generating") {
          setLlmProgress(t("generatingWithDiary"));
        }
      };
  ...
  const processTranscription = async (text: string) => {
    if (!text || text.trim() === "") {
      alert(t("couldNotHear"));
      setIsProcessing(false);
      return;
    }
    try {
      setLlmProgress(t("analyzingEmotion"));
      const sentimentResult = await analyzeSentiment(text);

      setLlmProgress(t("generatingReflection"));
  ...
  if (isPending) {
    return (
      <div className="w-full max-w-md h-32 bg-muted/50 animate-pulse rounded-full mx-auto" />
    );
  }
  if (!session) {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto relative">
      <div className="flex flex-col items-center justify-center p-10 bg-card/40 backdrop-blur-xl rounded-3xl shadow-2xl border border-black/10 dark:border-white/10 relative overflow-hidden transition-all duration-300 hover:shadow-primary/5 w-full max-w-lg">
        ...
        <h2 className="text-2xl font-semibold mb-3 text-foreground tracking-tight z-10">
          {t("title")}
        </h2>
          ...
          {isEditing ? (
            ...
            <div className="w-full min-h-[60px] bg-black/5 dark:bg-white/5 rounded-xl p-4 flex items-center justify-center text-center">
              <p className="text-sm text-foreground/80 break-words w-full">
                {transcript ||
                  (isRecording ? t("listening") : t("wordsAppearHere"))}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="relative flex items-center justify-center z-10 h-32 w-32">
          ...
          {!isRecording && !isProcessing && !isEditing ? (
            ...
            <Button
              disabled
              size="lg"
              className="relative rounded-full w-24 h-24 bg-muted text-muted-foreground border-white/10"
            >
        ...
        <div className="min-h-8 mt-6 z-10 flex flex-col items-center justify-center text-center">
          ...
          {isProcessing && (
            <div className="flex flex-col items-center gap-2">
              ...
              {llmProgress && (
                <p className="text-xs text-muted-foreground w-full max-w-sm mt-1">
                  {llmProgress}
                </p>
              )}
            </div>
