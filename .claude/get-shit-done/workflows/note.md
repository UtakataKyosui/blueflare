...
- **Global scope**: `/workspaces/sandbox/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/sandbox/.claude/notes/`)
...
2. Glob `/workspaces/sandbox/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/sandbox/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/sandbox/.claude/notes/` — works in any directory
